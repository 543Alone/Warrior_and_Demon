...
def get_companion_action(companion, party, enemy):
    ...
    available_skills = list(companion.get('skills', ['attack']))
    # 添加武器专属技能
    ...
    for s in available_skills:
        skills_with_cost.append(f"{s} (耗蓝: {cost_map.get(s, 20)} MP)")
...
【你可用的技能 (当前MP: {companion.get('mp', 0)}/{companion.get('max_mp', 100)})】
{json.dumps(skills_with_cost, ensure_ascii=False)}
如果选择的技能耗蓝大于你的当前MP，该操作将失败，并会被强制转为 attack！
