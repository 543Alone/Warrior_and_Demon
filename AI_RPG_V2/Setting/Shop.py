...
from Characters_intro.Bag import add_item_to_bag
...
def get_shop_items():
    ...
    for item in items_list:
        if item["name"] in ["小苹果", "强效治疗药水", "烤鸡腿", "魔法药水", "回城卷轴"]:
...
def sell_menu(player):
    while True:
        ...
        for p in Relo.party:
            all_equipped_ids.append(id(p.get('equipped_weapon')))
            all_equipped_ids.append(id(p.get('equipped_armor')))

        for original_idx, item in enumerate(player['bag']):
            is_equipped = id(item) in all_equipped_ids
        ...
        if not sellable_items:
            ...
            for i, info in enumerate(sellable_items):
                ...
                equipped_tag = f"{Colors.RED}[装备中 - 不可售卖]{Colors.END}" if info[
                    "is_equipped"] else f"(售价: 💰 {sell_price} G)"
        ...
        if choice.isdigit():
            ...
            if 0 <= idx < len(sellable_items):
                ...
                if info["is_equipped"]:
                    ...
                    print(f"✅ 卖出了 1 个 [{item['name']}]，获得 💰 {sell_price} G。")
            else:
                print("❌ 无效的编号！")
